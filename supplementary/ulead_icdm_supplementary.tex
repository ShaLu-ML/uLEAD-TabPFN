...
\begin{document}
	...
	For uncertainty-aware scoring, one variance network is trained per latent dimension for 50 epochs using Adam with a learning rate of $10^{-3}$ and weight decay of $10^{-4}$. 
	...
	Figure~\ref{fig:anomaly_attribution} shows the attribution of these latent-space dependency violations back to the original input features. Owing to the linear encoder, the contribution of each original feature to the anomaly score can be explicitly quantified. The strongest attributions correspond to features associated with vegetation condition, such as near-infrared reflectance (Mean-NIR) and panchromatic intensity variation (SD-Pan). Similar attribution patterns are consistently observed across other top-ranked Wilt anomalies.

	\begin{figure}[htbp]
		\centering
		\includegraphics[width=\columnwidth]{anomaly_01_attribution.png}
		\caption{Feature-level attribution of the anomaly score to the five original input features for anomaly~\#1 (highest-ranked Wilt anomaly). Bar heights reflect each feature's contribution to the total anomaly score via the linear encoder.}
		\label{fig:anomaly_attribution}
	\end{figure}
	
	We provide detailed numerical breakdowns for the highest-ranked anomaly (Index: 2361, Score: 75.36). Table~\ref{tab:anomaly1_values} reports the raw feature values, normalized values, and attribution scores for each original feature. Table~\ref{tab:anomaly1_latent} reports the per-latent-dimension analysis, including latent values, predicted conditional means, residuals, conditional variances, negative log-likelihoods, and per-dimension score contributions.

	\begin{table}[htbp]
		\centering
		\caption{Raw values, normalized values, and attribution scores for each original feature of anomaly~\#1 (highest-ranked Wilt anomaly, score: 75.36).}
	...
	\begin{table}[htbp]
		\centering
		\caption{Per-latent-dimension analysis for anomaly~\#1: latent values, predicted conditional means, residuals, conditional variances, negative log-likelihoods, and score contributions.}
		\resizebox{1\columnwidth}{!}{
			\begin{tabular}{lrrrrr}
				\toprule
