...
%% Supplementary Material - IEEE ICDM 2026 camera-ready paper
...
\begin{document}
	...
	\author{
		\IEEEauthorblockN{Sha Lu\IEEEauthorrefmark{1},
			Jixue Liu\IEEEauthorrefmark{1},
			Stefan Peters\IEEEauthorrefmark{1},
			Thuc Duy Le\IEEEauthorrefmark{1},
			Yongzheng Xie\IEEEauthorrefmark{2},
			Lin Liu\IEEEauthorrefmark{1}, and
			Jiuyong Li\IEEEauthorrefmark{1}}
		\IEEEauthorblockA{\IEEEauthorrefmark{1}Adelaide University, Adelaide, South Australia, Australia}
		\IEEEauthorblockA{\IEEEauthorrefmark{2}Queen's University Belfast, Belfast, United Kingdom}}

	\maketitle

	\noindent This document provides supplementary material accompanying the paper accepted at the 2026 IEEE International Conference on Data Mining (ICDM 2026).
	It is organized as follows.
	Appendix~\ref{app:datasets} provides dataset statistics for all 57 ADBench datasets.
	Appendix~\ref{app:baselines} describes all 28 baseline methods.
	Appendix~\ref{app:implementation} gives implementation details and hyperparameter settings.
	Appendix~\ref{app:pseudocode} presents pseudocode for the training and inference procedures.
	Appendix~\ref{app:results} reports complete per-dataset ROC-AUC and PR-AUC results, pairwise Wilcoxon signed-rank test results, and the Friedman/Nemenyi omnibus rank analysis.
	Appendix~\ref{app:case} presents a detailed case study on the Wilt dataset.
	Appendix~\ref{app:sensitivity} reports latent-dimension sensitivity results.
	Appendix~\ref{app:rcs_sensitivity} reports RCS-budget sensitivity results.
	...
	\label{app:datasets}
	Table~\ref{tab:datasets} summarizes all 57 ADBench datasets used in the experiments. They span a wide range of dimensionalities (3 to 1555 features), sample sizes (80 to 619326 instances), anomaly ratios, and application domains. Following the main paper, we group datasets into three dimensionality bands: low-dimensional ($d \leq 20$, 24 datasets), medium-dimensional ($20 < d \leq 100$, 18 datasets), and high-dimensional ($d > 100$, 15 datasets).
	...
	\begin{table*}[htbp]
		...
		\caption{Summary of the 57 ADBench datasets used for evaluation. Datasets are sorted by category. Low-dimensional: 24 datasets with $d \leq 20$; medium-dimensional: 18 datasets with $20 < d \leq 100$; high-dimensional: 15 datasets with $d > 100$.}
		\label{tab:datasets}
	\end{table*}
	...
	The encoder is trained for 100 epochs using Adam with no weight decay and a batch size of 1024, with the learning rate scheduled from $5\times10^{-4}$ to $10^{-6}$.
	For residual-scale normalization, one variance network is trained per latent dimension for 50 epochs using Adam with a learning rate of $10^{-3}$ and weight decay of $10^{-4}$.
	The RCS budget is set to 500. 
	The weights of the dependency loss $\lambda_{\mathrm{dep}}$ and the reconstruction loss $\lambda_{\mathrm{rec}}$ are set to 1 and 0.3, respectively.
	The training and inference procedures are presented in Algorithms~\ref{alg:train_ulead} and~\ref{alg:infer_ulead} in Appendix~\ref{app:pseudocode}.
	...
	Algorithms~\ref{alg:train_ulead} and~\ref{alg:infer_ulead} present the training and inference procedures for uLEAD-TabPFN, respectively.
	...
	\begin{algorithm}[t]
		...
		\begin{algorithmic}[1]
			...
			\State $L_{\mathrm{var}} \gets \frac{1}{2|\mathcal{B}|}\sum_{\mathbf{x}_i\in\mathcal{B}}
			\left[\frac{(z_{ij}-\hat{\mu}_{ij})^2}{\hat{\sigma}^2_{ij}+\epsilon} + \log \hat{\sigma}^2_{ij}\right]$
	...
	\begin{algorithm}[t]
		...
		\begin{algorithmic}[1]
			...
			\State $s(\mathbf{x}) \gets s(\mathbf{x}) + \frac{1}{2}\left[\frac{(z_{j}-\hat{\mu}_{j})^2}{\hat{\sigma}^2_{j}+\epsilon}+\log \hat{\sigma}^2_{j}\right]$
			\EndFor
	...
	\subsection{Friedman/Nemenyi Omnibus Rank Analysis}
	\label{app:friedman_nemenyi}

	The Wilcoxon results above provide dimensionality-stratified pairwise evidence.
	As a complementary omnibus test, the Friedman test evaluates whether all methods have equal rank distributions over the 57 datasets; a significant result licenses post-hoc pairwise comparisons.
	The Nemenyi post-hoc test then compares method ranks while explicitly accounting for multiple comparisons across all method pairs.

	Table~\ref{tab:friedman_nemenyi_summary} summarises the results.
	The Friedman test strongly rejects the null hypothesis of equal rank distributions for both ROC-AUC ($p \approx 3.76\times10^{-75}$, 28 methods) and PR-AUC ($p \approx 1.91\times10^{-78}$, 29 methods).
	Under the subsequent Nemenyi post-hoc test at $\alpha{=}0.05$, uLEAD-TabPFN achieves mean ranks of $10.84$ (ROC-AUC) and $11.18$ (PR-AUC) out of 28 and 29 methods, respectively.
	No compared method ranks significantly \emph{above} uLEAD-TabPFN, while 11 methods rank significantly \emph{below} it for both metrics.
	This result supports the conclusion that uLEAD-TabPFN belongs to the statistically leading group overall.
	The dimensionality-stratified Wilcoxon results in Table~\ref{tab:wilcoxon_pvalues} provide complementary pairwise evidence and show that the strongest significant gains occur in the high-dimensional regime ($d>100$).

	\begin{table}[t]
		\centering
		\caption{Friedman/Nemenyi rank-test summary over all 57 datasets. ``Better'' and ``Worse'' count methods ranked significantly above or below uLEAD-TabPFN under the Nemenyi post-hoc test at $\alpha{=}0.05$.}
		\label{tab:friedman_nemenyi_summary}
		\resizebox{\columnwidth}{!}{%
		\begin{tabular}{lccccc}
			\toprule
			Metric & Methods & Friedman $p$-value & uLEAD-TabPFN rank & Better & Worse \\
			\midrule
			ROC-AUC & 28 & $3.76\times10^{-75}$ & 10.84 & 0 & 11 \\
			PR-AUC  & 29 & $1.91\times10^{-78}$ & 11.18 & 0 & 11 \\
			\bottomrule
		\end{tabular}}
	\end{table}

	Figure~\ref{fig:friedman_nemenyi_rankplot} shows the critical-difference rank plot for the all-dataset Friedman/Nemenyi analysis.

	\begin{figure}[t]
		\centering
		\includegraphics[width=\columnwidth]{friedman_nemenyi_rankplot.pdf}
		\caption{Critical-difference rank plot for the Friedman/Nemenyi analysis over all 57 datasets.
		Lower mean rank is better.
		Horizontal bars connect groups of methods that are \emph{not} significantly different from each other under the Nemenyi post-hoc test at $\alpha{=}0.05$.
		Methods not connected to uLEAD-TabPFN by a bar are significantly different from it.
		uLEAD-TabPFN is highlighted; no method ranks significantly above it.}
		\label{fig:friedman_nemenyi_rankplot}
	\end{figure}

	\section{Detailed Case Study: Wilt Dataset}
	\label{app:case}
	...
	\begin{table}[htbp]
		...
		\resizebox{\columnwidth}{!}{%
			\begin{tabular}{p{3cm}ccp{5cm}}
				...
				& $0.3316$ & $0.3387$
				& The separation between normal and anomalous samples remains similar across spaces. \\
	...
	Figure~\ref{fig:wilt_umap} shows that normal and anomalous samples remain substantially overlapping in both the original and latent spaces, consistent with the near-zero silhouette scores reported in Table~\ref{tab:latent_distance_analysis} (from $-0.140$ in the original space to $0.002$ in the latent space).

	\begin{figure}[htbp]
		\centering
		\includegraphics[width=\columnwidth]{umap.png}
		\caption{
			UMAP visualization of the Wilt dataset in the original feature space (left) and the learned latent space (right). Normal samples are shown in gray and anomalous samples in color. Despite the latent transformation, normal and anomalous samples remain substantially overlapping; anomalous samples nevertheless form more locally coherent neighborhoods in the latent space.
		}
		\label{fig:wilt_umap}
	\end{figure} Likewise, the average distance from anomalous samples to their nearest normal neighbors changes only marginally, from $0.332$ to $0.339$, and density-based contrast remains weak, with the LOF ratio slightly decreasing from $1.038$ to $1.030$. Together, these results indicate that the improved detection performance of uLEAD-TabPFN is not driven by global geometric separation or marginal distance- and density-based cues.
	...
	We analyze a representative top-ranked anomaly from the Wilt dataset to illustrate this diagnostic. Its squared standardized deviations are substantially larger for $Z_0$, $Z_1$, $Z_2$, and $Z_4$ than for $Z_3$ (Table~\ref{tab:anomaly1_latent}). Mapping this evidence through the absolute linear-encoder weights yields the residual-based feature associations reported in Table~\ref{tab:anomaly1_values}; the strongest correspond to near-infrared reflectance (Mean-NIR) and panchromatic intensity variation (SD-Pan).
	
	We provide a numerical breakdown for the highest-ranked anomaly (Index: 2361). Table~\ref{tab:anomaly1_values} reports its raw and normalized feature values together with the residual-based attribution. Table~\ref{tab:anomaly1_latent} reports the per-dimension conditional quantities. The NLL row includes the conventional constant $\frac{1}{2}\log(2\pi)$, which is omitted from the implemented score because it does not affect rankings; the squared standardized-deviation row isolates the residual component of the NLL.

	\begin{table}[htbp]
		\centering
		\caption{Raw values, normalized values, and residual-based attribution scores for anomaly~\#1.}
	...
	\begin{table}[htbp]
		\centering
		\caption{Per-latent-dimension analysis for anomaly~\#1, including conventional Gaussian NLL values and their squared standardized-deviation components.}
		\resizebox{1\columnwidth}{!}{
			\begin{tabular}{lrrrrr}
				\toprule
	...
	Figure~\ref{fig:supp_latent_dim} reports the average ROC-AUC and PR-AUC over these datasets as $p$ varies: performance improves rapidly as $p$ increases from small values and stabilizes around $p \approx 40$, with only marginal gains for larger values up to $p = 100$.
	...
	\section{RCS-Budget Sensitivity}
	\label{app:rcs_sensitivity}
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\columnwidth]{rcs_budget_sensitivity.png}
		\caption{Sensitivity to the RCS budget under the final evaluation protocol. Increasing the context budget improves PR-AUC but substantially increases training time; the default budget $c=500$ provides a practical trade-off.}
		\label{fig:supp_rcs_budget}
	\end{figure}

	Figure~\ref{fig:supp_rcs_budget} reports sensitivity to the RCS budget. Increasing the budget from $c=100$ to $c=1000$ improves PR-AUC from 0.5089 to 0.5545, while ROC-AUC remains stable across the tested budgets, with values of 0.8093, 0.8080, 0.8078, and 0.8071 for $c=100$, $250$, $500$, and $1000$, respectively. However, training time increases sharply from 442.1\,s to 2797.4\,s overall, with the steepest rise between $c=500$ and $c=1000$: doubling the budget more than doubles the training time (1109.8\,s to 2797.4\,s), consistent with the increased cost of in-context TabPFN evaluation over larger context sets. The default budget $c=500$ achieves nearly the same PR-AUC as $c=1000$ (0.5509 vs.\ 0.5545) at substantially lower training cost, providing a practical accuracy-efficiency trade-off.
